...
internal sealed partial class FormPaintOverAndUnder : Form
{
    ...
    private void bitmapDisplay_PaintOver(object sender, PaintOverEventArgs e)
    {
        ...
        info.Append($"Display mode      {bitmapDisplay.DisplayMode.Humanize()}\n");
        info.Append($"Display size      {bitmapDisplay.ClientSize}\n");
        ...
        else
        {
            ...
            info.Append($"Bitmap size       {bitmapDisplay.DisplayImage?.Size}\n");
            info.Append($"Paint zoom        {bitmapDisplay.Zoom:0.000}\n");
            info.Append($"Paint rect        {r.X:0.0}, {r.Y:0.0}, {r.Width:0.0}, {r.Height:0:0}\n");
            info.Append($"Format            {bitmapDisplay.DisplayImage?.PixelFormat.Humanize()}\n");
        }
        info.Append($"Paint foreground  {bitmapDisplay.TimingMetrics.ForegroundPaint.Humanize()}\n");
        info.Append($"Paint background  {bitmapDisplay.TimingMetrics.BackgroundPaint.Humanize()}\n");
